        ...
        $definition->rootNode()
            ->children()
                ->enumNode('mode')
                    ->values(['move', 'copy'])
                    ->defaultValue('copy')
                    ->info('Formatter mode. "copy" (default) promotes ECS fields AND keeps the legacy Monolog top-level keys (channel, level_name, level, datetime) so existing dashboards keep working during migration; "move" drops them for a clean ECS-only shape. Both modes promote ECS fields identically.')
                ->end()
